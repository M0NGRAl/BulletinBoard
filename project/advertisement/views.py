...
class ResponseList(ListView):
    ...
    def get_queryset(self):
        ...
        # Применяем фильтр, передавая только объявления текущего пользователя
        self.filterset = ResponseFilter(self.request.GET, queryset=queryset)
        self.filterset.filters['advertisement'].queryset = advertisements  # Ограничиваем объявления
    ...
    def get_context_data(self, **kwargs):
        ...
        # Если self.filterset не был инициализирован, сделаем это сейчас
        if not hasattr(self, 'filterset'):
            self.filterset = ResponseFilter(self.request.GET, queryset=self.get_queryset())
